...
contract iTicketing is ERC721, ERC721URIStorage {
    ...
    uint256 public totalBNBFromTicket;
    uint256 private OrganizersBNBShare;
    ...
    event WithdrawAdmin(address admin, uint256 amount, uint256 withdrawal_time);
    event WithdrawPlatform(address admin, uint256 amount, address receiver, uint256 withdrawal_time);
       ...
       function supportsInterface(bytes4 interfaceId) public view virtual override( ERC721) returns (bool) {
        ...
        function register(uint256 _amount) external payable {
            ...
            if(eventDetails.eventFee == 0 && msg.value == 0) {
                ...
                hasRegistered[msg.sender] = true;
                eventDetails.totalParticipants = eventDetails.totalParticipants + 1;
                totalBNBFromTicket = totalBNBFromTicket + msg.value;
        ...
        function BNBBalanceOfOrganizer() external view returns(uint) {
            return OrganizersBNBShare;
        }
        ...
        function withdrawEventAdmin(uint256 _amount) external onlyEventAdminOrController {
            uint fee = calcQuotas();
            OrganizersBNBShare = totalBNBFromTicket - fee;

            if(!withdrawIsOpen) revert("Withdrawal closed. Contact controller");
            if(_amount > OrganizersBNBShare) revert Insufficient_funds();
            OrganizersBNBShare = OrganizersBNBShare - _amount;
            (bool success, ) = payable(msg.sender).call{value: _amount}("");
            require(success, "BNB transfer fail...");
            emit WithdrawAdmin(msg.sender, _amount, block.timestamp);
        }
        ...
        function withdraw() external {
            ...
            emit WithdrawPlatform(msg.sender, fee, factory, block.timestamp);
        }
        ...
        function calcQuotas() internal returns(uint256) {
            uint fee = (totalBNBFromTicket * 5) / 100;
